<header>
    <nav class="nav-bar">
        <!-- Hamburger Menu Icon -->
        <i class="ri-menu-line hamburger-icon" aria-label="Menu"></i>

        <!-- Logo -->
        <div class="logo"><a href="../index.php">TUB</a></div>

        <!-- Navigation Links -->
        <ul class="nav-menu">
            <li class="dropdown">
                <a href="#">Mens <i class="ri-arrow-down-s-line"></i></a>
                <ul class="dropdown-menu">
                    <li><a href="/Assignment1/products/men-all.php">View All</a></li>
                    <li><a href="#">Shirts</a></li>
                    <li><a href="#">Pants</a></li>
                    <li><a href="#">Shoes</a></li>
                </ul>
                
            </li>
            <li class="dropdown">
                <a href="#">Womens <i class="ri-arrow-down-s-line"></i></a>
                <ul class="dropdown-menu">
                    <li><a href="/Assignment1/products/women-all.php">View All</a></li>
                    <li><a href="#">Dresses</a></li>
                    <li><a href="#">Bags</a></li>
                    <li><a href="#">Accessories</a></li>
                </ul>
            </li>
            <li class="dropdown">
                <a href="#">Blog <i class="ri-arrow-down-s-line"></i></a>
                <ul class="dropdown-menu">
                    <li><a href="#">Fashion Tips</a></li>
                    <li><a href="#">Trends</a></li>
                </ul>
            </li>
            <li class="dropdown">
                <a href="#">About <i class="ri-arrow-down-s-line"></i></a>
                <ul class="dropdown-menu">
                    <li><a href="#">Our Story</a></li>
                    <li><a href="#">Contact</a></li>
                </ul>
            </li>
        </ul>

        <div class="nav-icons">
            <i class="ri-search-line"></i>
            <a href="/Web_Application/user/login.php"><i class="ri-user-line"></i></a>
            <i class="ri-shopping-bag-line"></i>
        </div>
        

    </nav>
    <hr class="divider">
</header>